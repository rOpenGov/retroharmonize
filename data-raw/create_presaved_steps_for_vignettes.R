...
save(ab_metadata, to_harmonize, R6,
  ...
  harmonized_ab_waves,
  file = file.path("inst", "afrob", "afrob_vignette.rda"),
  compress = T
)
...
ab_plot_ordered <- ab_plot2 %>%
  mutate(
    country_code = forcats::fct_reorder(
...
ggsave(
  filename = file.path("vignettes", "ab_plot1.png"),
  plot = ab_plot1,
