...
#' )
#'
#' # Variable-level harmonization function
#' harmonize_eb_trust <- function(x) {
#'   if (!inherits(x, "retroharmonize_labelled_spss_survey")) {
#'     return(x)
#'   }
#'
#'   label_list <- list(
...
harmonize_survey_values <- function(survey_list, 
  ...
  other_types <- all_names[names(classes)[classes == "other"]]
  ...
  to_harmonize_labelled <- lapply(
    ...
    extended,
    function(x) x %>% dplyr::select(all_of(retroharmonized))
  )
  ...
  fn_harmonize <- function(dat, .f) {
    ...
    dat %>%
      dplyr::select(-all_of(names(retroh))) %>%
      bind_cols(retroh) %>%
      dplyr::select(all_of(orig_name_order))
  }

  dat <- to_harmonize_labelled[[1]]

  rth <- lapply(
    to_harmonize_labelled,
  ...
  attr(return_value, "id") <- paste0(
    "Surveys: ",
    paste(original_attributes$id, collapse = "; ")
  )

  attr(return_value, "filename") <- paste0(
    "Original files: ",
