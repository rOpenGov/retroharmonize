...
climate3 <- climate2 %>%
  ...
  mutate(
    age_education_rec = as.factor(.data$age_education_rec),
...
demography_ctable_2 <- eb_climate_metadata %>%
  ...
  mutate(
    val_numeric_target = ifelse(.data$val_label_orig == "No full-time education", 14, .data$val_numeric_target),
