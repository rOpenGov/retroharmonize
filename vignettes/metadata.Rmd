...
```{r processlist}
metadata_surveys_create (survey_list = example_surveys)
```
...
subset_example_metadata <- example_metadata %>%
  filter ( grepl("trust", .data$label_orig) ) %>%
  filter ( grepl("european_parliament", .data$label_orig)) %>%
  select ( all_of(c("filename", "label_orig", "var_name_orig", "valid_labels", "na_labels", "class_orig")))
...
```
The metadata created by the `metadata_create()` and its version for multiple surveys, `metadata_surveys_create`, gives a first overview for the harmonization of concepts, the necessary harmonization of variable names and variable labels. In this case:
