---
title: "Working With The labelled_spss_survey Class"
...
```
Which inherits some properties from `haven::labelled()` which can be manipulated by the [labelled](https://cran.r-project.org/web/packages/labelled/vignettes/intro_labelled.html) package.
...
```
When used within a the modernized version of *data.frame*, `tibble::tibble()`, the summary of the variable contents prints in an informative way.
...
```
```{r, eval = FALSE}
#integer does not work for some reason.
c(sl1, 1:3)
```
...
sum (sl1, na.rm=TRUE)
```
Behind the hoods is stays of course the conversion to numeric, which can be used for other mathematical / statistical function. 
