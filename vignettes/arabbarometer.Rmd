...
```
Form these variables, we see that they are not set in a standard format (variable names should not have spaces, but rather underscores when separating words). We can fix that with the following chunk of code:
...
now let's take a look the standardization of the variables, and we can see the variables in standard format
...
NA_Harmonized <- data.frame(
  original_label = Missing_values,
...
merged_arabb <- lapply(merged_arabb, function(df) {
  df$evaluation_in_the_last_parliamentary_elections[is.na(df$evaluation_in_the_last_parliamentary_elections)==T] <- 99999
  return(df)
} )
