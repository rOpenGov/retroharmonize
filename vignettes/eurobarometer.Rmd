...
If there is a `99901` value, it refers to an invalid observation. The missing value range is not correctly identified. It should be made sure that if there is an unrecognized category label it goes into the missing range (and is marked that it was not recognized). See `harmonize_na_values()` for details on when the code `99901` is used.
