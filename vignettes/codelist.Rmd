...
The problem witht the SDMX Codelists is that they are designed for already aggregated statistical data, and they character codelist `id` variables. Survey software and question banks use integer ids for the same answer options.
...
Furthermore, Eurobarometer often uses characters in the Labels that should be prohibitted because most programming languages or software use them with a particular meaning. A particularly bad habit is the use of ; or , (which can be used as column delimiters in files), the $ sign which is an anchor in regex and a selector in R.
...
- Literature review on codelist harmonization.  While DDI does not seem to be focusing on it (maybe wrong), statistical agencies use standarized codelists, and I am sure they are standardizing the labels early on, on the questionnaire. Examples: EU-SILC (Panni), Eurobarometer, Afrobarometer, Arab Barometer (Daniel)
...
3. `codelist_create()` should be a new function that creates a codelist which considers  [SDMX Content-Oriented Guidelines (COG)](https://sdmx.org/?page_id=4345) and any guidance from DDI on Quesion banks, and the DDI Question Construct. The empty function is now in the `codelist.R` file, develop the documentation and the code there.
