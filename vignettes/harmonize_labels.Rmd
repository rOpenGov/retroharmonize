...
The function `harmonize_values()` solves the following problems:
...
The historic metadata, i.e. the earlier naming and coding of the variable does not have to match, it is added to all "inherited vectors".
...
As soon as you have only compatible variables with matching names in two data frames, you can bind them together in a way that their history is preserved.  You can do this with `vctrs::vec_rbind` or `dplyr::bind_rows()`. The generic `rbind()` will lose 
...
While dplyr's join functions may result in correct values, the metadata gets lost. A new join method will be developed.
