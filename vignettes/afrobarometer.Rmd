---
title: "Working With Afrobarometer"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Working With Afrobarometer}
...
The Afrobarometer version of the trust variables is a bit different from Eurobarometer, it has 4 categories, not 2. We could just to map them into two, or give them an equi-distant numerical representation. This is what we do.

```{r}
labelled::val_labels(merged_ab[[1]]$trust_president)
```
...
harmonize_ab_trust <- function(x) {
  label_list <- list(
    from = c("^not", "^just", "^somewhat",
             "^a", "^don", "^ref", "^miss"), 
    to = c("not_trust_2", "not_trust_1", "trust_1", 
           "trust_2", "do_not_know", "declined", "inap"), 
...
```{r}
harmonized_waves <- harmonize_waves ( waves = merged_ab ) %>%
  mutate ( year = ifelse (grepl("r6_data_2016", unique_id), 
                          2016, 2019)) %>%
  mutate_at ( vars(starts_with("trust")), harmonize_ab_trust ) 
```
